...
public class Contour extends Plot {
    ...
    class Isoline {
        ...
        void paint(Graphics g) {
            ...
            if (label == null) {
                ...
                label = new Label(String.format("%s", level), horizontalReference, verticalReference, angle, coord);
            }
    ...
    public static PlotCanvas plot(double[] x, double[] y, double[][] z, double[] levels) {
        double[] lowerBound = {Math.min(x), Math.min(y)};
        double[] upperBound = {Math.max(x), Math.max(y)};
        PlotCanvas canvas = new PlotCanvas(lowerBound, upperBound, false);
        canvas.add(new Contour(x, y, z, levels));
        return canvas;
    }
